...
本产品使用到的训练方法，对业内的常用方式进行了改进，能极大提高软件测试领域的大模型的应用水平。我们计划使用专利的方式对其进行保护，本章节主要研究了比较可能的专利方向。

## 7.1 一种软件测试行业的大语言模型训练方法

随着大语言模型技术的发展，结合不同的行业知识，训练专用大语言模型，可以极大提高AI在专用领域工作的准确率。在软件测试领域，特别是端到端测试中，现有的训练方法还存在诸多问题，有的方法收集训练数据的方式很繁琐，有的需要大量的算力，有的方法准确率达不到实用要求。我们采用一种全新的专门为“端到端软件测试”的数据设计的训练方式，可以比较好的解决上面的问题，训练出来的行业大模型，比普通方法训练出来的大模型在“软件测试”领域编写的“测试用例”有更高的覆盖率。

![software-testing-llm-training-flowchart](pic/software-testing-llm-training-flowchart.svg)

### 数据收集

训练大模型最重要的部分，就是输入准确有效的数据，如果要训练专用大模型来预测测试用例，则会涉及到大量的界面跳转和交互元素。测试行业常用的数据有：
...
现有的大模型训练需要输入文本作为训练材料，而上述的“跳转”和“交互”并不容易变成合适的训练的文本材料并不容易。通常，大家的做法是，通过人工来为操作和每个界面和每个交互元素（按钮，输入框，菜单，等）设置编码，最后变成脚本语言来输入。这个过程非常繁琐，导致很难大量产生可以使用的训练数据。我们研发出了一种专门对应软件界面的识别算法，可以比通用模型更加准确和快速的识别出哪些是功能输入区（功能按钮），哪些是信息反馈区（标签、数值展示等），并且还能智能的建立起功能按钮的动作与信息反馈区之间的对应联动关系，无需人工参与，只需要给出一个软件的入口，就可以自己快速生成整个软件的训练数据。

### 模型训练

通过前面的章节描述的微调的方式来训练模型。避免全部参数的调整来大幅提高训练速度。我们计划使用LoRA（Low-Rank Adaptation）技术对预训练的大模型进行行业特定的微调，特别是针对 End-to-End Testing 领域的微调。这种方法允许在少量数据的情况下，快速高效地调整大模型的权重，使其在处理测试任务时具有更高的准确性和效率。相较于传统的大模型训练方法，这种微调方法显著降低了训练成本，并且能够在特定领域内提供更为精确的模型输出。

### 奖励机制

我们通过算法来完成大部分的繁琐的工作。使用一个专门设计的算法来作为训练过程中的奖励机制，以促使模型快速收敛：

- 正确性奖励：当算法意识到模型能够正确识别和操作软件界面元素时，给予正向奖励。例如，如果模型能够根据文档中的指示成功导航到特定的菜单项，那么模型将获得奖励。
- 错误惩罚：当模型操作错误或未能达到预期结果时，给予负向惩罚。例如，如果模型未能根据文档中的步骤正确填写表单，那么模型将受到惩罚。
- 探索奖励：为了鼓励模型探索新的操作和路径，当模型尝试新的操作时，即使结果不是最优的，也可以给予一定的奖励。
- 多样性奖励：鼓励模型在测试过程中覆盖不同的功能和操作路径，以确保测试的全面性。
- 效率奖励：对于能够快速完成任务的模型，给予额外的奖励，以提高测试的效率。

通过我们的算法作出的判断和奖惩，模型可以在训练过程中学习如何更有效地进行软件测试，同时减少错误和提高覆盖率。这种端到端的训练方法允许模型从输入到输出直接学习，无需人为干预，从而提高了自动化测试的效率和准确性。不但高于普通的通用大模型的能力，还能达到甚至高过初级测试人员的测试用例编写覆盖率。
